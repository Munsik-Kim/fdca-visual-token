"""Event-addressed RNG registry for the P3S checkpoint replication."""
...
EXPERIMENT_ID = "FDCA_VIS_P3S_SECOND_CHECKPOINT_REPLICATION"
...
class DiskSemanticRegistry:
    ...
    def uniform(self, parts: Mapping[str, Any], *, intentional_shared: bool = False) -> tuple[float, str]:
        ...
        self.all_handle.write(raw)
        if intentional_shared:
            self.allowed_handle.write(raw)
        return int.from_bytes(raw[:8], "big") / float(1 << 64), raw.hex()

    def close(self) -> None:
        if not self.all_handle.closed:
            self.all_handle.close()
        if not self.allowed_handle.closed:
            self.allowed_handle.close()
    ...
    def audit(self) -> dict[str, Any]:
        ...
        au, ac = (np.unique(allowed_values, return_counts=True)
                  if len(allowed_values) else (np.array([], dtype="S32"), np.array([], dtype=int)))
        ...
        return {
            "schema": "FDCA_VIS_P3S_RUNTIME_SEMANTIC_KEY_REGISTRY_V1",
